// FIXME: Wave 3 — replace with real EditorProjectStore reads / engine
// pack manifest selectors.
//
// This module exists so the Phase 2 panel-body agents have a stable
// shape to import dummy entity-definition data from while the real
// selectors are still in flight. Each export is a const, narrowed
// where it helps inference; row shapes are exported as `interface`s
// so panel code can reference them directly. When the real store
// lands, this file gets deleted in one go and the panel imports get
// repointed at the `editorProjectStore` selectors.
//
// Engine context: "Prefabs" are pack-defined ENTITY DEFINITIONS —
// reusable templates (Wolfenstein-style enemies / items / triggers /
// lights) that the Scene canvas instantiates by reference. The
// Prefabs page is the FORM-HEAVY EDITOR for these templates; the
// Scene page is where instances are placed. The page was renamed from
// "Entities" → "Prefabs" but the in-page domain noun remains "entity"
// to match the engine's runtime vocabulary.

// ---------------------------------------------------------------------------
// Component model
//
// An entity (prefab) is a bag of `ComponentInstance`s. Each instance
// has a `kind` (which schema in `COMPONENT_SCHEMAS` drives its
// editor controls), an `enabled` flag (so the user can toggle a
// component off without deleting it), and a free-shape `data` record
// whose keys correspond to the schema's `fields`.
//
// Wave 2's `ComponentEditorPanel` will read this row + the matching
// schema to render the right control for each field.
 ...
 *  the eyebrow label above each row group. Wave 2 may evolve these. */
...
// Component schemas
//
// For each `ComponentKind` we describe the editor controls the
// `ComponentEditorPanel` should render — labels, control type, range
// bounds for sliders / number inputs, select options, etc. Wave 2 walks
// this schema to lay out the per-component card's body.
//
// `description` strings double as progressive-tooltip bodies — they
// appear on the long-hover stage of each field's tooltip.
