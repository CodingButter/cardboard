...
/**
 * User-facing settings modal. Pack-side after R4. The `live` /
 * `overlay` / `onChange` / `onClose` contract is unchanged; the only
 * additions are `bindings` (label resolver) and `settings` (export +
 * import flows) — both passed in as props so the component itself
 * stays free of engine imports.
 *
 * `bindings.label(code)` replaces the direct `bindingLabel` import the
 * pre-R4 engine-side version used; `settings.export(...)` /
 * `settings.import()` replace the `exportSettings` / `importSettings`
 * direct imports. Same behaviour, same UX, different plumbing.
 */
