...
| `docs/plans/LIGHTING_OVERHAUL.md` | Bake-heavy emissive lighting model with dynamic-light overlay. Phase 7 absorbs the former LIGHTING_ENTITIES_REFACTOR (lights-as-entities) plan. | Phases 1, 2, 4, 5 shipped + split-lightmap polish + jittered LOS. Phase 3 (per-wall samples) pending. Phase 7 R1+R2 shipped via ENGINE_PACK_SPLIT R1; R3+R4 pending. |
...
| `docs/plans/EDITOR_ENGINE.md` | Editor Engine — shell + editor packs on pack-chain. Dock-type catalog, JSON-declarative panels, bundled npm libraries, Extensions tab, dev-mode injection. Synthesizes dogfooding + editor-package-injection + IDB + DnD + remote-dock memories. | Phases 0–5 shipped (Phase 4 = core-editor-pack extraction via CORE_EDITOR_PACK.md; Phase 3 = Visual Builder via JSON_VISUAL_BUILDER.md; Phase 5 = Performance Profiler proof). |
| `docs/plans/CORE_EDITOR_PACK.md` | Core editor-pack extraction — `apps/editor/src/views/` evaporates; "the Cardboard editor" itself becomes `packages/core-editor-pack/`. Proof-by-construction of the dogfooding principle. Phase 4 of EDITOR_ENGINE. | P1–P5 fully shipped (`6e23bf7` → `70cd29d`, 2026-05-20). React externalization spike → `registerPanel` API → 24 panels migrated in batches A/B/C/D → view shells + tab/view/layout registries → final cleanup. Followed by the post-extraction dead-code sweep (`da75e9d`, 3,120 LOC removed). The Cardboard editor now loads as a `.apg` from a bare shell. |
...
| `docs/plans/AUDIO.md` | Web Audio gain graph + `api.audio.{play,playLoop,playReplace,stop,stopAll}` + 5-group mixer + spatial audio. Sounds declared in `manifest.audio`. | Au1 shipped (commit `52d8e27`); Au2 (spatial + music + ducking), Au3 (editor authoring), Au4 (reverb) pending. |
...
| `docs/plans/TUTORIALS.md` | In-editor guided tutorials — step-by-step overlays driven by pack-declared lessons. | T1 (schema + spotlight overlay + EmptyState launcher), T2 (pack-shipped tutorials via pack-chain), T5 (5 built-in tutorials: Prefabs / Extensions / Panel Builder / Home + intro-scene) shipped. T3 (first-run auto-launch chain) + T4 (remaining built-ins blocked on IL/SL/UB) pending. |
| `docs/plans/JSON_VISUAL_BUILDER.md` | Drag-and-drop editor-panel builder. Ships as `cardboard-visual-builder-pack`. Phase 3 of EDITOR_ENGINE arc. | VB1–VB6 fully shipped (workspace skeleton → palette + canvas → selection + inspector → undo/redo + save/export/import → store-path/script-ref pickers → JSON mode + error boundary + polish). Plan landed + builder complete on 2026-05-20. |
...
| Materials M1+M2+M3+M5: per-entity Shader component + cell materials + scene overrides + build-time validation | ✅ Done (commit `bdab12a`) — see git log for the materials plan (shipped 2026-05-16) |
...
| Scene Wave 3 (cross-window store sync): 3.1 sync utility + 3.2 eight stores + 3.3 all 14 panels off MOCK fixtures + 3.4 MapCanvas painting + undo/redo replay | ✅ 2026-05-19/20 — `apps/editor/src/state/sync.ts` (`createSyncedStore` with persist + storage events + per-store BroadcastChannel), eight stores (tool/brush/tile-preset/layer/selection/scene/history/diagnostics), IDB→Zustand bridge on project-load (`hydration.ts`), tile-preset color path + texture rendering (`tileTextureCache.ts` + `useTilePresetRegistryStore.ts`), bulk `paintCells`/`eraseCells` (1 storage event per stroke), history dispatcher with `applyEntryUndo`/`Redo` + `undoOnce`/`redoOnce`, BFS flood-fill, brush footprint. See `docs/plans/SCENE_WAVE_3_WIRING.md`. 3.5 (popout validation) in progress. |
| Sidecar PWA + D10/D10b pairing: PeerJS cloud signaling + QR/URL pair modal + sidecar-side identity wizard + welcome/hello handshake + paired-peer chip | ✅ 2026-05-19 — `apps/editor/sidecar/` (cold-launch QR scanner + identity wizard + service worker + Tailwind), `desktopPairingSingleton.ts` (non-serializable Peer + DataConnection live outside Zustand), `PairDeviceModal.tsx` (qrcode lib + copy-URL fallback), `scripts/build-sidecar-for-docs.ts` (5-step pipeline staging into `apps/docs/public/sidecar/`). PeerJS tree-shake bug fixed via proper ESM `import { Peer } from "peerjs"` + peer-deps (`peerjs-js-binarypack`, `webrtc-adapter`, `@msgpack/msgpack@2.x`). See `docs/plans/REMOTE_DOCK_QR.md`. |
| Sidecar M2 — `mountPanel` consumer + touch-friendly panel variants | ✅ 2026-05-20 (`9517565`) — sidecar consumes the `mountPanel` WireMessage end-to-end via `apps/editor/sidecar/lib/panelRegistry.tsx` (`resolveSidecarPanel(kind)`) + `MountedPanelScreen.tsx`. Three touch-friendly panel kinds wired: `tools`/`brush` → ToolsPanel, `tilePresets`/`colorPicker`/`palette` → ColorPickerPanel, `notes`/`scratchpad` → NotesPanel. Unknown kinds render a friendly fallback. D12b device-chip drag target landed `40af88d`. D11+ store-sync over channel + D10d Supabase Realtime transport (task #11 delta) + D11b initial IDB mirror + D12 recent-sessions reconnect pending. See `docs/plans/REMOTE_DOCK_QR.md` §0. |
| Tutorial system T1–T2 + T5 — JSON tutorial format + SVG spotlight overlay + pack-chain authored tutorials + 5 built-in tutorials | ✅ 2026-05-20 — T1 (`59344e6`) ships the JSON tutorial schema, `runtime.ts`, SVG-mask spotlight `TutorialOverlay.tsx`, EmptyState `tutorial?: string` launcher; T2 (`91df815`) walks `<pack>/tutorials/*.tutorial.json` from the pack-chain so packs ship their own lessons; T5 (`218c174`) lands four more built-in tutorials (Prefabs / Extensions / Panel Builder / Home) on top of the intro-scene seed. Files under `apps/editor/src/tutorials/`. T3 (first-run auto-launch chain) + T4 (remaining built-ins, dependent on IL/SL/UB stable `data-tutorial` attributes) pending. See `docs/plans/TUTORIALS.md` §10. |
| Editor Engine — Phase 3 Visual Builder (`cardboard-visual-builder-pack`) | ✅ 2026-05-20 — VB1 (`d28e917`) workspace skeleton + Panel Builder tab + 3-pane shell; VB2 (`2566d12`) palette + canvas + `RenderSpec` node (recursive `PanelRenderer` verified); VB3 (`6e1b12f`) selection + property inspector; VB4 (`819f49b`) undo/redo + IDB sidecar save/export/import + Panel Library dock panel; VB5 (`19591ec`) Custom node + store-path/script-ref pickers walking live registries; VB6 (`683259a`) JSON mode + error boundary + polish. Authoring loop closes — a non-coder can produce a `.json` panel spec without touching a text editor. See `docs/plans/JSON_VISUAL_BUILDER.md`. |
| Post-extraction dead-code cleanup — `apps/editor/src/` deorphaning | ✅ 2026-05-20 (`da75e9d`) — 3,120 LOC removed, 20+ orphan files deleted after the core-editor-pack extraction (P1–P5) absorbed the old `apps/editor/src/views/` tree. The shell now ships only what `CORE_EDITOR_PACK.md` §2 enumerates. |
| Editor Engine architecture (dogfooding pivot) — Phases 0, 1, 2, 4, 5 | ✅ 2026-05-19/20 — designed 2026-05-19; Phase 0 (JSON panel renderer) + Phase 1 (panel-renderer migrations) + Phase 2 (pack-bundled libraries + Extensions tab) + Phase 4 (core-editor-pack extraction, see CORE_EDITOR_PACK row) + Phase 5 (Performance Profiler demo pack, `0c5519b`) all shipped. Shell now ships only primitives + Tailwind defaults + Zustand sync + IDB + pack-chain loader + extension manager. |
| Editor Engine — Phase 4 core-editor-pack extraction (P1–P5) | ✅ 2026-05-20 — `6e23bf7` (P1 React externalization spike) → `205f1f6` (P2 `registerPanel` API + NotesPanel) → `10ed2ac`/`8c747da`/`69290c`/`72d5b1c`/`0a006f5`/`5b4742f` (P3 batches A–final, 24 panels migrated) → `70fc96b` (P4 view shells + tab/view/layout registries) → `70cd29d` (P5 final cleanup) → `f82a470` (shell view migration endpoint). `apps/editor/src/views/` no longer exists; the Cardboard editor loads as a `.apg` from a bare shell. See `docs/plans/CORE_EDITOR_PACK.md`. |
