...
- **Skeletal animation.** No bones, no IK, no inverse kinematics —
  ...
  in the default pack stays unused.
...
6. **`api.events` dependency for A2.** Hard dependency. Until `api.events` ships in MULTIPLAYER M1, A1 uses the internal-map fallback for `onComplete`. Update MULTIPLAYER_PLAN's M1 phase note when A2 lands.
