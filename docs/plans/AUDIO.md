...
|---|---|---|---|
| **Au1** | **Core surface.** `manifest.sounds` schema + `SoundDef`. `api.audio.play` / `playLoop` / `playReplace` / `stop` / `stopAll`. Web Audio gain graph (master + 5 groups). Lazy AudioContext bootstrap with one-shot user-gesture resume. `GameConfig.audio` + Settings sliders (live). Default-pack ships 2 sample sounds (gunshot + pickup chime) wired into the existing gun-render + pickup systems. | engine/src/Audio/, engine/src/ModAPI/types.ts, engine/src/GameConfig.ts, engine/src/AssetPack/types.ts, default-pack/audio/, default-pack/scripts/systems/gun-render.js + pickup.js | Designed. Not started. |
| **Au2** | **Spatial audio + music.** `playPositional` with PannerNode + HRTF. Listener auto-tracking. `crossfadeMusic`. Modal-open ducking via `api.modals.any()` driving `masterGain × CONFIG.audio.modalDuck`. Default-pack adds an ambient bed + theme music + battle music. | engine/src/Audio/AudioBackend.ts (PannerNode path), AudioSystem.ts (listener tracking + ducking), default-pack/audio/{music,ambient}/ | Designed. Depends on Au1. |
